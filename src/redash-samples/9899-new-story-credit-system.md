...
blurb: A new credit tracking system rolled out in 2020; how's that going?<br/><br/><span class="note">A note about the last part of the WHERE clause -- since the snapshot is a date integer in YYYYMMDD format, the <code>% 10000</code> modulo is a convenient way to look just at the month and day. <code>415</code> is April 15, <code>615</code> is June 15, <code>815</code> is August 15, etc. Presto would need a large amount of memory to load in all the snapshots and do the distinct story_id count. Using a subset of snapshots allows the query to stay within memory limits.</span>
